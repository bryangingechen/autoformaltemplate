...
# rebuild (see Enharmonic/CLAUDE.md *Build discipline*; added after
# the 2026-06-10 OOM crash).
#
...
if printf '%s' "$stripped" | grep -qE 'lake[^|;&]*--update|lake +update'; then
  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Blocked: lake update / lake --update rewrites lake-manifest.json and lean-toolchain and forces a from-source mathlib rebuild. Toolchain and dependency bumps are a human decision; see Enharmonic/CLAUDE.md (build/lint gates)."}}'
fi
exit 0
